...
# 检查是否启用了 napcat 或 llbot 模式
if [[ "${MODE:-}" != "napcat" && "${MODE:-}" != "llbot" ]]; then
    log_info "MODE 不是 napcat 或 llbot，跳过配置"
    exit 0
fi

log_info "开始配置 ${MODE} 适配器"
...
DICE_QQ="${DICE_QQ:-}"
if [[ -z "$DICE_QQ" ]]; then
    log_info "环境变量 DICE_QQ 未设置，尝试从 /app/napcat/config 目录查找"
    if [[ -d "/app/napcat/config" ]]; then
        # 查找 onebot11_数字.json 文件，按修改时间排序，取最新的
        latest_file=$(find /app/napcat/config -name "onebot11_*.json" -type f 2>/dev/null | xargs ls -t 2>/dev/null | head -n1)
        ...
        log_warn "/app/napcat/config 目录不存在"
    fi
fi

# 如果仍未设置，尝试从现有配置文件读取
if [[ -z "$DICE_QQ" ]]; then
    log_info "尝试从现有 serve.yaml 读取骰子QQ"
    if [[ -f "data/default/serve.yaml" ]]; then
        # 查找所有QQ平台的适配器
        existing_qqs=$(yq '.imSession.endPoints[] | select(.baseInfo.platform == "QQ") | .baseInfo.userId | capture("QQ:(?<qq>\\d+)").qq' "data/default/serve.yaml" 2>/dev/null | head -1)
        if [[ -n "$existing_qqs" ]]; then
            DICE_QQ="$existing_qqs"
            log_info "从现有配置文件中提取到骰子QQ: $DICE_QQ"
        else
            log_warn "现有配置文件中未找到QQ平台适配器"
        fi
    else
        log_warn "serve.yaml 文件不存在"
    fi
fi
...
EOF
        ...
        # 删除所有QQ平台的配置（保留其他平台如微信、Discord等）
        log_info "清理所有QQ平台配置"
        # 获取所有QQ平台端点的索引（从后往前删除）
        qq_indices=$(yq ".imSession.endPoints | to_entries | map(select(.value.baseInfo.platform == \"QQ\")) | map(.key) | .[]" "$SERVE_FILE" 2>/dev/null | sort -rn | tr '\n' ' ')
        
        if [[ -n "$qq_indices" ]]; then
            for idx in $qq_indices; do
                yq -i "del(.imSession.endPoints[$idx])" "$SERVE_FILE"
            done
            log_info "已删除所有QQ平台配置"
        fi
